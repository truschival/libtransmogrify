...
class LibtransmogrifyRecipe(ConanFile):
    ...
    def get_git_describe_rev(self):
        ...
        git_describe_rev =  git.run("describe --tags --long --abbrev=8 --always --dirty").strip()
    ...
    def get_semver_from_git(self):
        ...
        if not match:
            raise Exception(f"Cannot parse git describe output '{git_describe_rev}' to semver version!")
    ...
    def export(self):
        self.output.info("Exporting recipe - updating conandata.yml with git describe revision")    
        git_describe_rev = self.get_git_describe_rev()
    ...
    def build(self):
        ...
        cmake.configure(
            variables={
                ...
                "CONAN_GIT_DESCRIBE_REV": self.conan_data.get("git_describe_rev", "XXX"),
            }
        )
        cmake.build()
